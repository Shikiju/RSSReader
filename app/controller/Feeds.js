...
Ext.define('RSS.controller.Feeds', {
	...
	onBlockPoster: function(e, e1, e2, e3){

		/* Warning the user that this will redirect him to an external
		 * url outside the application asking him if he wants to continue. */
		Ext.Msg.confirm('BlockUser', 'Are you sure you want to block this user??', function(btn){
			
			console.log(this);
			
			console.log(Ext.getStore('News').getAt(this.newsIndex));
			
			//The answered yes so...
			if(btn == 'yes'){
				//console.log(this);
				//console.log(Ext.getStore('BlockedUsers'));
			}
